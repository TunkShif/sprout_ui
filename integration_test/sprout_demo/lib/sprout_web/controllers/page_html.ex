defmodule SproutWeb.PageHTML do
    ...
    <Display.accordion :let={api} items={@items}>
      <div :for={item <- api.items} class="group" {item.container_attrs}>
        ...
        <div
          ...
          data-transition
          data-enter="transition-[height] duration-300 ease-in-out"
          data-enter-from="h-0"
          data-enter-to="h-[var(--panel-height)]"
          data-leave="transition-[height] duration-300 ease-in-out"
  ...
  def rendered_component(%{component: "collapsible"} = assigns) do
    ~H"""
    <Display.collapsible :let={api}>
      <button
      ...
      >
        Collapse
        <svg
          class="ui-open:rotate-180 w-2.5 h-2.5 text-white"
          width="16"
          height="16"
          viewBox="0 0 16 16"
          fill="none"
